...
const DetailGudang = () => {
  ...
  const [transactionHistoryWarehouse, setTransactionHistoryWarehouse] =
    useState([]);
  const [loadingTrxHistoryWarehouse, setLoadingTrxHistoryWarehouse] =
    useState(false);
  ...
  const handleReason = () => {
    axios
      .get("https://digiwarehouse-app.onrender.com/dasboard/payment/reasone", {
        headers,
      })
      .then((response) => {
  ...
  const handleInstallment = (id) => {
    ...
    axios
      .get(
        `https://digiwarehouse-app.onrender.com/payment/instalment/${id}?page=1&limit=10`,
        { headers }
      )
      .then((response) => {
                ...
                <table className="w-full text-sm text-left rtl:text-right mt-3 text-cloud-burst-500">
                  ...
                  {loadingTrxHistoryWarehouse ? (
                    ...
                    <>
                      ...
                      transactionHistoryWarehouse.length > 0 ? (
                        <tbody>
                          {transactionHistoryWarehouse &&
                            transactionHistoryWarehouse.map((trx, index) => (
                              <tr
                                key={index}
                                onClick={() => {
                                  handleInstallment(trx.transactionId);
                                }}
                                className="cursor cursor-pointer"
                              >
                                <th className=" py-3 font-medium text-cloud-burst-500 whitespace-nowrap">
                                  {index + 1}
                                </th>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center">
                  <div className="bg-white p-5 rounded-md w-auto relative overflow-auto max-h-screen">
                    ...
                    {installment.map((instalmentItem, index) => (
                      <div className="text-cloud-burst-500" key={index}>
                        <div className="grid grid-cols-2 ">
                          <h1 className="mt-2 mb-2 text-[20px] text-cloud-burst-500 font-semibold">
                            {new Intl.DateTimeFormat("id-ID", {
                              month: "long",
                            }).format(new Date(instalmentItem.dueDate))}
                          </h1>
                          <p className="text-center ml-[150px] mt-1 text-cloud-burst-500 font-bold">
                            {instalmentItem.nominal.toLocaleString("id-ID", {
                              style: "currency",
                              currency: "IDR",
                            })}
                          </p>
                          ...
                          <button
                            className={`${
                              instalmentItem.status === "belum dibayar"
                                ? "bg-red-500"
                                : "bg-green-500"
                            } py-2 px-2   rounded-md text-white ml-[150px] -mt-2`}
                          >
                            {instalmentItem.status}
                          </button>
                        </div>
                    ...
                    <div className="flex gap-2 justify-end">
                      <button
                        className="rounded-md bg-cloud-burst-200 py-2 px-5 text-white"
                        onClick={handelModalStopKontrak}
                        id="showModalStopKontrak"
                      >
                        Berhenti Kontrak
                      </button>
                      <button
                        className="rounded-md bg-cloud-burst-500 py-2 px-5 text-white"
                        onClick={handleModalUpdateTempo}
                        id="showModalUpdateTempo"
                      >
                        Perpanjang Tempo
                      </button>
                    </div>
              ...
              {showModalStopKontrak && (
                <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center">
                  <div className="bg-white p-5 rounded-md w-[880px] relative overflow-auto max-h-screen">
                    <h1 className="text-xl text-cloud-burst-500 font-bold mb-1 relative">
                    ...
                    <p className="text-cloud-burst-500 text-xl mt-6">
                      Kontrak Sewa diberhentikan dengan Alasan
                    </p>
                    {reason &&
                        ...
                        <div key={index} className="mt-4">
                          <label className="">
                            <input
                              ...
                              value={reason.name}
                            />
                            <span className="ml-4 text-cloud-burst-500">
                              {reason.name}
                            </span>
                          </label>
                        </div>
                    ...
                    <div className="flex justify-end mt-8 gap-2 ">
                      <button
                        className="rounded-md bg-crusta-200 text-white px-6 py-3"
                        id="cancelStopKontrak"
                      >
                        Batal
                      </button>
                      <button
                        className="rounded-md bg-crusta-500 text-white px-6 py-3"
                        id="stopKontrak"
                      >
                        Hentikan Kontrak
                      </button>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center">
                  <div className="bg-white p-5 rounded-md w-[525px] relative overflow-auto max-h-screen">
                    ...
                    <div className="flex flex-col justify-center">
                      <h2 className="text-center text-xl text-cloud-burst-500">
                        Tempo tagihan diperpanjang hingga:
                      </h2>
                      <input
                        type="datetime-local"
                        name=""
                    ...
                    <div className="flex justify-end gap-3 mt-3">
                      <button
                        className="border-2 border-cloud-burst-500 bg-white rounded-lg py-3 px-6  font-semibold text-cloud-burst-500"
                        onClick={() => setShowModalUpdateTempo(false)}
                        id="cancelUpdateTempo"
                      >
                        Batalkan
                      </button>
                      <button
                        className="border-2 border-cloud-burst-500 bg-cloud-burst-500 rounded-lg py-3 px-6 text-white font-semibold"
                        id="submitUpdateTempo"
                      >
                        Submit
                      </button>
                    </div>
