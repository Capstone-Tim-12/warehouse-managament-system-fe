...
const DetailUser = () => {
  ...
  const handleTransactionHistory = (userId) => {
    ...
    axios
      .get(`https://digiwarehouse-app.onrender.com/dasboard/user/${userId}/transaction?page=1&limit=10`, { headers })
      .then((response) => {
  ...
  const handleViewDetail = (transactionId) => {
    ...
    axios
      .get(`https://digiwarehouse-app.onrender.com/dasboard/user/transaction/${transactionId}`, { headers })
      .then((response) => {
...
<div className='mt-6'>
<h1 className='text-xl text-cloud-burst-500 font-bold'>Riwayat Penyewaan Gudang</h1>
        ...
        {transactionHistory && transactionHistory.map((transaction, index) => (
          <tr key={index} className="cursor-pointer" onClick={() => handleViewDetail(transaction.transactionId)}>
            ...
            <td className="px-6 py-4">
              {transaction.rentalDuration} {transaction.paymentScheme}
            </td>
            <td className="px-6 py-4">
              {transaction.warehouseName}
            </td>
            <td className="px-6 py-4">
              {transaction.paymentTotal.toLocaleString("id-ID", {
                      style: "currency",
                      currency: "IDR",
                    })}
            </td>
...
{/* End Riwayat Penyewaan Gudang */}
          ...
          <div className='catatan'>
            <h2 className='text-[16px] text-cloud-burst-500 font-bold mt-4'>Catatan</h2>
            <input type="text"
      ...
      <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center" >
        <div className="bg-white p-5 rounded-md w-auto relative overflow-auto max-h-screen">
          ...
          <tbody >
          <tr><td className=' py-4'>Nama Gudang</td><td>:</td><td>{selectedTransaction.warehouseName}</td></tr>
          <tr><td className=' py-4'>Nama Penyewa</td><td>:</td><td>{selectedTransaction.username}</td></tr>
          <tr><td className=' py-4'>Status</td><td>:</td><td>Tanpa Ktp</td></tr>
          <tr><td className=' py-4'>Alamat</td><td>:</td><td>{selectedTransaction.address}</td></tr>
          <tr><td className=' py-4'>Durasi Sewa</td><td>:</td><td>{selectedTransaction.duration} {selectedTransaction.paymentScheme}</td></tr>
          <tr><td className=' py-4'>Tanggal Masuk</td><td>:</td><td>{new Intl.DateTimeFormat('id-ID', { year: 'numeric', month: 'long', day: 'numeric' }).format(new Date(selectedTransaction.entryDate))}</td></tr>
          <tr><td className=' py-4'>Tanggal Keluar</td><td>:</td><td>{new Intl.DateTimeFormat('id-ID', {year: 'numeric', month:'long', day:'numeric'}).format(new Date(selectedTransaction.outDate))}</td></tr>
          <tr><td className=' py-4'>Total Pembayaran</td><td>:</td><td>{selectedTransaction.totalPayment.toLocaleString("id-ID", {
                      style: "currency",
                      currency: "IDR",
                    })}</td></tr>
          <tr><td className=' py-4'>Status Pembayaran</td><td>:</td><td></td></tr>
          </tbody>
          </table>
          ...
          <p className='mb-4 '>{instalment.paymentName || 'Belum Bayar'}</p>
          <p className='text-center ml-[150px] mt-1'>{instalment.nominal.toLocaleString("id-ID", {
                      style: "currency",
                      currency: "IDR",
                    })}</p>
          <p>1 Januari 2023</p>
          <button className='bg-green-500 py-2 px-10 rounded-md text-white ml-[150px] -mt-2'>{instalment.status}</button>
          </div>
          </div>
          ))}
          {/* end instalment */}
