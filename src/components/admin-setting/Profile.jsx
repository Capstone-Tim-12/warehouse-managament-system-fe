...
const endpointUpdate =
  "https://digiwarehouse-app.onrender.com/dasboard/user/setting";
...
const Profile = () => {
  ...
  const handleSubmit = () => {
    setUpdating(true);
    axios
      .put(endpointUpdate, profile, { headers })
      .then(() =>
  ...
  const validate = (e) => {
    ...
    return !profile.password ? setIsPasswordError(true) : handleSubmit();
  };
  ...
  return (
    <div className="flex flex-col gap-11">
      ...
      <div
        className={`flex flex-row items-center m-auto px-4 gap-2 h-16 rounded-lg w-full focus-within:border-cloud-burst-500 border ${
          profile.username !== "" && "border-cloud-burst-500 border"
        }`}
      >
        <div>
        ...
        <div className="flex flex-col w-full relative">
          <label className="relative cursor-pointer mt-1">
            ...
            />
            <span className="absolute left-0 transition duration-200 label-input">
              Nama
            </span>
          </label>
        </div>
      ...
      <div
        className={`flex flex-row items-center m-auto px-4 gap-2 h-16 rounded-lg w-full focus-within:border-cloud-burst-500 border ${
          profile.email !== "" && "border-cloud-burst-500 border"
        }`}
      >
        <div>
      ...
      <div className="flex flex-col gap-2">
        <div
          className={`flex flex-row items-center m-auto px-4 gap-2 h-16 rounded-lg w-full focus-within:border-cloud-burst-500 border ${
            profile.password !== "" && "border-cloud-burst-500 border"
          }`}
        >
          <div>
        ...
        <p className="text-red-400">
          {isPasswordError && "Masukkan Password!"}
        </p>
      </div>
      <div className="flex justify-end">
        <button
          ...
          className={`bg-crusta-500 text-white flex justify-center items-center h-10 w-72 rounded-lg hover:bg-crusta-600 ${
            isUpdating ||
            (isPasswordError && "bg-crusta-500 opacity-50 cursor-not-allowed")
          }`}
        >
          Simpan
        </button>
